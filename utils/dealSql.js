...
function getList (connection, sql, req, res) {
    var query = req.query
    connection.query(sql.editSql(req).getCount, (err0, counts, fields0) => {
        connection.query(sql.editSql(req, query.page).getList, (err, results, fields) => {
            if (err) {
                res.json({status: err})
            } else {
                res.json({status: 0, data: results, page: Number(query.page), pages: Math.ceil(counts[0].count / (query.size || 10))})
            }
        })
 ...
 * */
exportData.prototype.editSql = function (req, page = 1, offset = 10) {
